...
const clansPage = createApp({
    ...
    methods: {
        ...
        getClanImage(clanId) {
            const logo = this.shield.find(shield => shield.id === clanId);
            return logo ? logo.imagen : '';
        },
        goToClanDetails(clanId) {
            window.location.href = `detalleClanes.html?id=${clanId}`;
        }
    },
    computed: {
        filteredClans() {
            ...
            return this.clans.filter(clan => clan.name.startsWith(this.selectedLetter));
        }
    }
}).mount("#containerVue");
